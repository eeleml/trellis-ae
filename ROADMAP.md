...
- **`closed-lost` refined (v0.9.15)** — the angle now reads the **full** lost-reason picture (`closed_lost_category` + `closed_lost_reason_1`/`_2` + the free-text `closed_lost_reason` comment + `closed_lost_reason_comment_product` + `closed_lost_reason_comment_competitor`/`who_we_lost_to`), and cross-references a new **`config/whats-new.md`** release timeline against the deal's close date to surface *"what we shipped since you passed"* (only releases postdating the last contact, matched to the product-gap reason; never fabricated). Added an **in-chat preview** of the angle + E1 before it drafts to Gmail. Still the shared `ob-messaging` `closed_lost` branch (one place to tune). **`whats-new.md` ships as a template — Ethan must populate real releases.**
